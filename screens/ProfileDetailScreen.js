...
const ProfileDetailScreen = ({ route, navigation }) => {
  ...
  const getFlagColor = (flag) => {
    if (['trustworthy', 'responsive', 'genuine', 'verified', 'helpful', 'community_leader', 'trusted', 'friendly', 'active', 'new_user'].includes(flag)) {
      return '#4CAF50';
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Flags */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Flags & Indicators</Text>
          <View style={styles.flagsContainer}>
            {profile.flags.map((flag, index) => (
              <View key={index} style={styles.flagItem}>
                <Ionicons 
                  name={getFlagIcon(flag)} 
                  size={16} 
